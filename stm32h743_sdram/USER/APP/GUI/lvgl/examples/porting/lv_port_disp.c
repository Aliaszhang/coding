 ...
 *      DEFINES
 *********************/
// #define LTDC_LCD_FRAME_SIZE         (1280*800*2)
 ...
 *  STATIC VARIABLES
 **********************/
static lv_color_t color_buf[COLOR_BUF_SIZE] __attribute__((section(".bss.EXRAM1")));
static lv_color_t color_buf2[COLOR_BUF_SIZE] __attribute__((section(".bss.EXRAM1")));

static uint16_t color_tmp[COLOR_BUF_SIZE*2] __attribute__((section(".bss.EXRAM1")));
...
void lv_port_disp_init(void)
{
    ...
    lv_disp_buf_init(&draw_buf_dsc_1, color_buf, color_buf2, LV_HOR_RES_MAX * LV_VER_RES_MAX);   /*Initialize the display buffer*/
...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    for(y = area->y1; y <= area->y2; y++) {
        for(x = area->x1; x <= area->x2; x++) {
            ...
            /* put_px(x, y, *color_p)*/
            // *(uint16_t *)((uint32_t)ltdc_framebuf[lcdltdc.activelayer]+lcdltdc.pixsize*(lcdltdc.pwidth*(lcdltdc.pheight-area->x1-x-1)+area->y1+y)) = color_p->full;
...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    memcpy(color_tmp, color_p, COLOR_BUF_SIZE*2);

    LCD_Color_Fill((uint16_t)area->x1, (uint16_t)area->y1, (uint16_t)area->x2, (uint16_t)area->y2, (uint16_t *)color_tmp);
    lv_disp_flush_ready(disp_drv);
}
#endif
